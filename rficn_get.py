...
l='https://www.rfi.fr/cn/%E6%BB%9A%E5%8A%A8%E6%96%B0%E9%97%BB/'
l2='https://www.rfi.fr'
d=str(datetime.today()-timedelta(days=1)).split(' ')[0]
if not os.path.exists('index.list'):
    h=rg.rget(l).text
    s=bs(h,'html.parser')
    hl=[{'source':'%s%s'%(l2,a.find('a').get('href')),
         'publisher':a.find('div',{'class':'news__metadata__source'}).string}
        for a in s.find('div',{'class':'o-layout-list o-layout-list--news-feed o-layout-list--timeline'}).find_all('div',{'class':'o-layout-list__item'})]
    f=open('index.list','w+');f.write(json.dumps(hl));f.close()
else:
    f=open('index.list','r');hl=json.loads(f.read());f.close()
...
if len(dr)==0:
    ...
    for a in range(len(hl)):
        ...
        mt={'title':'og:title',
            ...
            'modified time':'article:modified_time'}
        ...
        for o in vs:
            ...
            o.replace_with(nno)
        ps=[]
        try:st=[z for z in s.find('div',{'class':'t-content__body u-clearfix'}).contents if z]
        except:
            #t-content__chapo
            st=[z for z in s.find('p',{'class':'t-content__chapo'}).contents if z]
        nst=[]
        st=[str(z)for z in st]
        for z in st:
            if z.find('© 2023 AFP')!=-1:
                break
            if z.find('© 2023 AFP')!=-1:
                break
            nst.append(z)
        ...
        st=''.join(nst)
        si=s.find('div',{'class':'t-content__main-media'}).find('img')
        si['src']=si['srcset'];del si['srcset']
        sn=''.join([str(si),st])
        s=bs(sn,'html.parser')
        invalid_tags=['div']
        ...
        des=['m-block-ad__label__text','m-interstitial__unlock-btn__text']
        for y in des:
            for x in s.find_all('span',{'class':y}):
                x.decompose()
...
for a in os.walk('JSON-src'):
    for b in a[2]:
        ...
        #'source', 'time', 'publisher', 'title', 'description', 'published time', 'modified time', 'videos', 'text', 'images'
        t='''# %s
