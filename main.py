...
ds='/media/a/aa0c6e82-c513-458a-ba7f-a7d591ff95fc/worlnews.github.io/Maoism-Datasets/%s'%str(datetime.date.today()+datetime.timedelta(days=0))
pds='/media/a/aa0c6e82-c513-458a-ba7f-a7d591ff95fc/worlnews.github.io/Maoism-Datasets/%s'%str(datetime.date.today()+datetime.timedelta(days=-1))
ud='/media/a/aa0c6e82-c513-458a-ba7f-a7d591ff95fc/worlnews.github.io'
...
for a in ps:
    if not os.path.exists(p:='src/%s'%a.replace('_',' ')[:-4]):
        ...
        for b in os.walk(sys.path[0]):
            for c in b[2]:
                if b[0]==sys.path[0]:
                    if c[-5:]in['.list','.json']or c=='RecentPostings.md.bin'or c=='test.htm'or c=='test.txt':
                        l.append(c)
...
for a in os.walk('src'):
    for b in a[2]:
        ...
        if('Images'in a[0])and b1:
            ...
            f=open(('HTMs/%s-%s.htm'%(b[:-4],str(n1).rjust(6).replace(' ','0'))).replace(':','-').replace('+','-'),'w+');f.write(tt.replace('../Images','../Images/%s'%pn).replace('../ConvertedIMGs','../ConvertedIMGs/%s'%pn));f.close()
        elif('MDs'in a[0])and b4:
            pn=a[0].split('/MDs')[0].split('/')[-1]
            n2+=1
            f=open(pa,'r');tt=f.read();f.close()
            f=open(('MDs/%s-%s.md'%(b[:-3],str(n2).rjust(6).replace(' ','0'))).replace(':','-').replace('+','-'),'w+');f.write(tt.replace('../Images','../Images/%s'%pn).replace('../ConvertedIMGs','../ConvertedIMGs/%s'%pn));f.close()
for a in os.walk('src'):
    if not a[2]:
